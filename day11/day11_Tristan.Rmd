...
load.RData("data/spatialSim.RData")
...
```{r}
transect <- read.csv("day11/data/transect.txt", header = TRUE, sep = " ")
skim(transect)
head(transect)
...
ggplot(data.frame(h = h, cov = cov_vals), aes(x = h, y = cov)) +
  geom_line() +
  labs(title = "Exponential covariance function", x = "h", y = "Covariance") +
  theme_minimal()
```
...
```{r}
# Inverse of sigma
sigma.inv <- solve(sigma.new)

lambda <- sigma.inv * (sigma.new + 1 * (1 - 1 * sigma.inv * sigma.new) / (1 * sigma.inv * 1))
```