...
bliss %<>% mutate(total = alive + dead)


logit_model <- glm(cbind(alive,dead)~conc, family = binomial(link=logit), data = bliss)
...
plot(alive/total~conc, xlim = c(-1,5), ylim = c(0,1), data=bliss)
...
plot(alive/total~conc, xlim = c(-1,5), ylim = c(0,1), cex=2, data=bliss)
...
probit_model <- glm(cbind(alive,dead)~conc, family = binomial(link=probit), data = bliss)
